...
public abstract class SanguinisLuxuriaConversionsProvider implements DataProvider {
    ...
    public CompletableFuture<?> run(DataWriter writer) {
        ...
        this.generateConversions(new ConversionExporter() {
            @Override
            public void export(ConversionJsonBuilder.Provider provider) {
                providers.add(provider);
            }

            @Override
            public String getDefaultNamespace() {
                return SanguinisLuxuriaConversionsProvider.this.modId;
            }
        });
        ...
        for (ConversionJsonBuilder.Provider provider : providers) {
            ...
            ConditionJsonProvider.write(json, DatagenConditions.consumeConditions(provider));
            futures.add(DataProvider.writeToPath(writer, json, this.getOutputPath(provider.getId())));
        }
        return CompletableFuture.allOf(futures.toArray(CompletableFuture[]::new));
    }

    protected ConversionExporter withConditions(ConversionExporter exporter, ConditionJsonProvider... conditions) {
        Preconditions.checkArgument(conditions.length > 0, "Must add at least one condition.");
        return new ConversionExporter() {
            @Override
            public void export(ConversionJsonBuilder.Provider provider) {
                DatagenConditions.addConditions(provider, conditions);
                exporter.export(provider);
            }

            @Override
            public String getDefaultNamespace() {
                return exporter.getDefaultNamespace();
            }
        };
    }

    protected abstract void generateConversions(ConversionExporter exporter);
    ...
    public interface ConversionExporter {
        void export(ConversionJsonBuilder.Provider provider);
