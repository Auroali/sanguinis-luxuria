...
public class SLPatchouliBooks extends PatchouliProvider {
    ...
    protected void generate() {
        ...
        PatchouliJsonCategory abilities = pack.category(PatchouliJsonCategory.create(
            "abilities",
            "Ritual of Transmutation", "Variations of the Ritual of Transmutation",
            SLItems.ALTAR
          ))
          .parent(new Identifier(SanguinisLuxuria.MODID, "rituals"))
          .sort(1);

        ritualEntries(rituals, abilities);
    ...
    private static void ritualEntries(PatchouliJsonCategory rituals, PatchouliJsonCategory abilities) {
        ...
        abilities.entry("blink", "Blink", Items.ENDER_PEARL)
          .advancement(SLResources.id("unlock/grow_tree_and_become_vampire"))
          .page(PatchouliTextPage.create()
            .title("Blink")
            .text("Blink allows the user to perform a short-range teleport. Mutually exclusive with $(l:mist)Mist$(/l).")
            .text("Activate with $(keybind:key.sanguinisluxuria.activate_blink)$()")
          )
          .page(PatchouliRitualPage.create(SLResources.id("rituals/blink")));

        abilities.entry("bite", "Bite", SLItems.VAMPIRE_FANG)
          .advancement(SLResources.id("unlock/grow_tree_and_become_vampire"))
          .page(PatchouliTextPage.create()
            .title("Bite")
            .text("Grants the ability to bite other creatures, inflicting bleeding.")
            .text("Activate with $(keybind:key.sanguinisluxuria.activate_bite)$()")
          )
          .page(PatchouliRitualPage.create(SLResources.id("rituals/bite")));

        abilities.entry("mist", "Mist", Items.COBWEB)
          .advancement(SLResources.id("unlock/grow_tree_and_become_vampire"))
          .page(PatchouliTextPage.create()
            .title("Mist")
        ...
        abilities.entry("infectious", "Infectious", PotionUtil.setPotion(new ItemStack(Items.POTION), Potions.POISON))
          .advancement(SLResources.id("unlock/grow_tree_and_become_vampire"))
          .page(PatchouliTextPage.create()
            .title("Infectious")
            .text("Grants the ability to transfer any active potions to another upon draining their blood.")
          )
          .page(PatchouliRitualPage.create(SLResources.id("rituals/infectious")));

        abilities.entry("vulnerability", "Vulnerability", Items.GLASS_PANE)
          .advancement(SLResources.id("unlock/grow_tree_and_become_vampire"))
          .page(PatchouliTextPage.create()
            .title("Vulnerability")
            .text("Grants increased strength and movement at a cost.")
            .text(
              "With Vulnerability, you are more susceptible to both the sun and any vampire-effective damage.",
              "Mutually exclusive with $(l:resilience)Resilience$(/l)"
            )
          )
        ...
        abilities.entry("resilience", "Resilience", Items.LEATHER_HELMET)
          ...
          .page(PatchouliTextPage.create()
            .title("Resilience")
            .text("Grants increased resistance against vampire-effective damage, as well as allowing you to stay out in the sun longer.")
            .text(
              "However, this comes at the cost of reduced health, movement speed, and attack speed.",
              "Mutually exclusive with $(l:vulnerability)Vulnerability$(/l)"
            )
          )
          .page(PatchouliRitualPage.create(SLResources.id("rituals/resilience")));
    }
}
