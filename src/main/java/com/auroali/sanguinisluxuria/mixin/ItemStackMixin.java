...
public abstract class ItemStackMixin {
    ...
    public Multimap<EntityAttribute, EntityAttributeModifier> sanguinisluxuria$modifyAttributeModifiers(Multimap<EntityAttribute, EntityAttributeModifier> original, @Local(argsOnly = true) EquipmentSlot slot) {
        if (slot != EquipmentSlot.HEAD || !this.isIn(SLTags.Items.SUN_BLOCKING_HELMETS) || original.containsKey(SLEntityAttributes.SUN_RESISTANCE))
            return original;

        return ImmutableMultimap
          .<EntityAttribute, EntityAttributeModifier>builder()
          .putAll(original)
          .put(SLEntityAttributes.SUN_RESISTANCE, new EntityAttributeModifier("Sun Resistance", 0.75d, EntityAttributeModifier.Operation.MULTIPLY_BASE))
          .build();
    }
}
