package com.auroali.sanguinisluxuria.common;

public class BloodConstants {
    public static final int BLOOD_PER_BOTTLE = 2;
    public static final int BLOOD_GAIN_RATE = 4800;
    public static final int BLOOD_DRAIN_TIME = 25;
    // todo: figure out a (good) way to determine if an entity has bleeding on the client, because apparently it doesnt sync ;-;
    public static final int BLOOD_DRAIN_TIME_BLEEDING = 15;
}
